...
def display_DR(args, new_X, X, phi, mu, W):
    if args.data == "Swiss_Roll" or args.data == "toy_data":
        ...
        if args.method == "PCA":
            ...
            print(len(np.matmul(W, new_xx)))
            print(len(np.matmul(W, new_xx)[0]))
            print(len(mu))
            print(len(mu[0]))
...
def PCA(X, out_dim):
    ...
    mu = np.arange(D, dtype=float).reshape(D, 1)
    cov_mat = []
    for x in range(D): mu[x][0] = np.mean(X[x, :], axis = 0)
    for i in range(D): cov_mat.append(X[i, :])    
